"""Score aggregated TA results into personality trait summaries."""
...
def add_likert_values(df: pd.DataFrame) -> pd.DataFrame:
    """Compute categorical and weighted Likert summaries."""
...
def reverse_code(row: pd.Series) -> pd.Series:
    """Reverse-code negatively keyed items in place."""
    inv = row["inventory"]
    if inv == "BFI10":
...
def _score_bfi10(batch: pd.DataFrame, col: str) -> dict[str, float]:
    """Score a BFI-10 batch into OCEAN trait means."""
    out = {c: np.nan for c in SCORE_COLUMNS}
    by_trait = {t: [] for t in OCEAN_SHORT}
    for _, r in batch.iterrows():
        trait = next(
            e["trait"][0] for e in BFI10_ITEMS_TRAIT_POLARITY if e["item"] == r["item"].strip()
        )
...
def _score_panasx(batch: pd.DataFrame, col: str) -> dict[str, float]:
...
def _score_ipip120(batch: pd.DataFrame, col: str) -> dict[str, float]:
...
def _dispatch(inv: str, batch: pd.DataFrame, col: str) -> dict[str, float]:
    """Dispatch inventory scoring to the appropriate helper."""
    if inv == "BFI10":
...
def score_inventories(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate inventory batches into scored output rows."""
