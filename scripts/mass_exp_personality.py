"""Schedule batch personality experiments while respecting cluster time limits."""
...
def is_cluster_environment() -> bool:
    """Return True when the SLURM scheduler is available."""
...
def get_remaining_time() -> int | None:
    """Return the remaining wall-clock time reported by SLURM."""
...
def get_time_estimate(use_peft: bool | None) -> float:
    """Estimate runtime from the recorded history for the current mode."""
...
def update_time_stat(use_peft: bool | None, elapsed: float) -> None:
    """Persist a runtime sample for the current experiment mode."""
    key = "lora" if use_peft else "none"
...
def generate_experiment_id_from_values(
    dataset: str | None,
    split: str | None,
    model_name: str | None,
    seed: int,
    ...
    """Build a stable experiment identifier from the configuration values."""
    dataset_abbr = dataset[0].upper() if dataset else "d"

    def abbreviate_split(split: str | None) -> str:
        if split:
            parts = split.split("-")
            if len(parts) >= 3:
...
splits = [
    f"{trait}-{location}-{size}"
    for trait, location, size in itertools.product(traits, locations, sizes)
]


def run_experiment(
    params: tuple[str, int, int, int, str | None, int | None],
    index: int,
    total: int,
) -> None:
    """Run one scheduled personality experiment."""
    ...
    if is_cluster_environment():
        ...
        if remaining < estimated_time + TIME_BUFFER:
            print(
                f"[SKIPPED] Not enough time left: {remaining}s vs needed {estimated_time + TIME_BUFFER}s"
            )
            return
    else:
    ...
    print(
        f"[RUNNING] split={split} | seed={seed} | batch={batch_size} | grad_steps={grad_step} | peft={use_peft} | subset={subset}"
    )
...
def main() -> None:
    """Enumerate and run the personality experiment grid."""
    combinations = list(
        itertools.product(
            splits,
            seeds,
