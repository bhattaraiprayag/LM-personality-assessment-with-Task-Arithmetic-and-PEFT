"""Sweep base/LoRA pairs across adapter scales and export scored results."""
...
from experiment_config import peft_scales
...
def collect_experiment_summary(meta_path: str, outputs_base: str) -> pd.DataFrame:
    """Load experiment metadata and locate associated checkpoints."""
    ...
    for exp_id, exp in meta.items():
        ...
        if use_peft == "lora":
            ...
            model_files = [
                f for f in files if f.endswith(("model.safetensors", "config.json", ".bin"))
            ]
...
def make_combinations(df: pd.DataFrame) -> pd.DataFrame:
    """Build paired baseline/LoRA combinations for scoring."""
...
def load_combined_model(
    ...
    """Load a baseline model and attach the LoRA adapter weights."""
    ...
    state = torch.load(
        os.path.join(lora_row.exp_out_dir, lora_path), map_location="cpu", weights_only=True
    )
...
def main(args: argparse.Namespace) -> None:
    """Run the full sweep and write the aggregated results CSV."""
    ...
    for combo_idx, (_, cmb) in enumerate(df_combos.iterrows(), 1):
        ...
        for scale in tqdm(peft_scales, desc="Evaluating across LoRA scales..."):
            with rescale_adapter_scale(model, scale):
                for inv in ("BFI10", "PANASX", "IPIP120"):
                    df_inv = evaluator.score_likert(
                        inventory_name=inv,
