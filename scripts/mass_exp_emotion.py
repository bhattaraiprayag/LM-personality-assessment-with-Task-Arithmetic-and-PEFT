"""Schedule batch emotion experiments while respecting cluster time limits."""
...
TIME_STATS_PATH = os.path.join("outputs", "time_stats_emotion.json")
TIME_BUFFER = 30  # 0.5 minutes buffer to avoid overlap
...
def get_time_estimate(use_peft: bool | None) -> float:
    ...
    if not times:
        return 180  # default to 10 minutes if no prior data
...
def run_experiment(
    ...
    """Run one scheduled emotion experiment."""
...
def main() -> None:
    """Enumerate and run the emotion experiment grid."""
    combinations = list(
        itertools.product(
            splits,
            seeds,
