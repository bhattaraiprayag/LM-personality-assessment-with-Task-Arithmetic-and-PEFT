"""Create fine-tuning visualizations from scored evaluation outputs."""
...
def mean_sd(df: pd.DataFrame, value: str, by: list[str]) -> pd.DataFrame:
    """Compute grouped mean and standard deviation for a value column."""
...
def preprocess_baseline(df: pd.DataFrame) -> pd.DataFrame:
    """Filter and normalize the baseline scoring table."""
    df = df.copy()
    df = df[
...
def preprocess_results(df: pd.DataFrame) -> pd.DataFrame:
    """Filter and normalize the fine-tuning results table."""
    df = df.copy()
...
def plot_personality_h1(b_base: pd.DataFrame, res: pd.DataFrame, inv: str, outdir: Path) -> None:
    """Plot pre-trained versus fine-tuned personality scores for H1."""
    ...
    for letter in OCEAN_LETTERS:
        ...
        for loc in LOCATIONS:
            ...
            means = [base_val] + [
                agg.loc[s, "mean"] if s in agg.index else np.nan for s in SPLIT_SIZES
            ]
            ...
            ax.set_ylabel("Score")
            title = (
                f"{LETTER2NAME[letter]} | Pre-trained vs. Finetuned ({loc.title()} splits) – {inv}"
            )
            ax.set_title(title)
            fig.tight_layout()
...
def plot_emotion_h1(b_base: pd.DataFrame, res: pd.DataFrame, outdir: Path) -> None:
    """Plot pre-trained versus fine-tuned emotion scores for H1."""
    ...
    for split in [e.lower() for e in EMOTION_TRAITS]:
        ...
        ax.bar(
            x - width / 2,
            [base_vals[t] for t in EMOTION_TRAITS],
            width,
            color="red",
            label="Pre-trained",
        )
        ax.bar(
            x + width / 2,
            [post_vals[t] for t in EMOTION_TRAITS],
            width,
            color="green",
            label="Finetuned",
        )
...
def plot_personality_h2(res: pd.DataFrame, inv: str, outdir: Path) -> None:
    """Plot LoRA-scale sweeps for personality scores in H2."""
    for letter in OCEAN_LETTERS:
        trait_lower = LETTER2LOWER[letter]
        for loc in LOCATIONS:
            sub = res[
            ...
            ax.set_ylabel("Score")
            ax.set_title(
                f"{LETTER2NAME[letter]} scores | LoRA Scales – {loc.title()} splits ({inv})"
            )
...
def plot_emotion_h2(res: pd.DataFrame, outdir: Path) -> None:
    """Plot LoRA-scale sweeps for emotion scores in H2."""
    for split in [e.lower() for e in EMOTION_TRAITS]:
        sub = res[
            (res["dataset"] == "emotion")
            & (res["use_peft"] == "lora")
        ...
        for i, t in enumerate(EMOTION_TRAITS):
            ...
            ax.fill_between(
                xs, ys - agg["sd"], ys + agg["sd"], alpha=0.15, color=colors[i % len(colors)]
            )
        style_symlog(ax)
        ax.set_xlabel("LoRA Scale")
        ax.set_ylabel("Score")
