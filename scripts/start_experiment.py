"""Run a full training, evaluation, and export cycle."""
...
def main() -> None:
    """Run the experiment pipeline and save all outputs."""
    ...
    results["train_metrics"] = {
        k: v.item() if isinstance(v, torch.Tensor) else v for k, v in train_metrics.items()
    }
    if test_results:
    ...
    if args.use_peft:
        results["custom_eval_files"]["mid"] = os.path.join(evals_dir, "mid_epoch_results.csv")
