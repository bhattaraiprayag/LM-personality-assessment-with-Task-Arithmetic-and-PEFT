"""Create H3 analysis plots from scored inventory outputs."""
...
def style_symlog(ax) -> None:
    """Apply a symmetric logarithmic x-axis style."""
...
def parse_model(mid: str) -> dict:
    """Return a normalized metadata dict for a model identifier."""
    default = dict(
        dataset="unknown",
        trait="unknown",
        location="unknown",
    ...
    if mid_l.startswith("p-"):
        m = re.match(r"^p-([ocean])([tb])(\d+)", mid_l) or re.match(r"^p-([a-z])([tb])(\d+)", mid_l)
        if not m:
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize scored results for H3 plotting."""
    df = df.copy()
    df["lora_scale"] = df["lora_scale"].replace("baseline", 0).astype(float)
    df = df[
        (df["likert_in_prompt"] == VARIANT_LIKERT_INCLUSION)
        & (df["variant_likert"] == VARIANT_LIKERT_SCORING)
    ]
...
def plot_h3_1(df: pd.DataFrame, outdir: Path) -> None:
    """Plot combined base and LoRA scores for each inventory."""
    for (bm, lm), g in df.groupby(["base-model", "lora-model"]):
        base_model_info = parse_model(bm)
        lora_model_info = parse_model(lm)
        title_part = f"Base: {base_model_info['desc_full']} + LoRA: {lora_model_info['desc_full']}"
        for inv, sub in g.groupby("inventory"):
            ...
            for col in traits:
                ...
                ax.fill_between(
                    ms["lora_scale"], ms["mean"] - ms["sd"], ms["mean"] + ms["sd"], alpha=0.15
                )
            style_symlog(ax)
            ax.set_ylabel("Score")
            ...
            ax.legend()
            fname = (
                f"{variant_suffix}_h3.1_{inv}_{base_model_info['desc_short']}_"
                f"{lora_model_info['desc_short']}.png"
            )
            fig.tight_layout()
...
def gather_variable(
    df: pd.DataFrame, side: str, trait: str, loc: str, const_id: str
) -> pd.DataFrame:
    """Filter rows where one model side varies across a trait split."""
    if side == "b":
        f = (
...
def is_pandora(info: dict) -> bool:
    """Return True when the parsed model came from the Pandora dataset."""
    return info["dataset"] == "pandora"


def fmt_pandora(info: dict) -> str:
    """Format a short label for a Pandora split or another model."""
    if is_pandora(info):
        return f"{info['trait'].lower()}-{info['location']}"
    return info["desc_short"]


def plot_h3_2(df: pd.DataFrame, outdir: Path) -> None:
    """Plot cross-dataset comparisons for the mixed-model experiments."""
    id_pairs = (
        ...
        for inv, g in inv_groups:
            ...
            for tr in [var_trait, const_trait]:
                ...
                if ax.lines:
                    ...
                    fname = f"{variant_suffix}_h3.2_{inv}_{base_desc}_{lora_desc}_{tr.lower()}.png"
                    fig.tight_layout()
                    fig.savefig(outdir / fname, bbox_inches="tight")
                plt.close(fig)
