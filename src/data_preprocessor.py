...
class DataPreprocessor:
    ...
    def __init__(
        ...
        self.splits_folder_balanced = splits_folder_balanced or os.path.join(data_folder, 'splits_balanced/')
        self.ocean_traits = ocean_traits or ['agreeableness', 'openness', 'conscientiousness', 'extraversion', 'neuroticism']
        self.mbti_traits = mbti_traits or ['introverted', 'intuitive', 'thinking', 'perceiving']
    ...
    def filter_authors_with_profiles(self):
        ...
        self.filtered_comments = self.comments[self.comments['author'].isin(common_authors)]
    ...
    def filter_authors_with_complete_traits(self):
        ...
        self.ocean_comments = self.filtered_comments[self.filtered_comments['author'].isin(ocean_complete_authors)]
        self.mbti_comments = self.filtered_comments[self.filtered_comments['author'].isin(mbti_complete_authors)]
        ...
        self.all_traits_comments = self.filtered_comments[self.filtered_comments['author'].isin(all_complete_authors)]
        ...
        print(f"Total comments from authors with all OCEAN traits: {len(self.ocean_comments)}")
        print(f"Total comments from authors with all MBTI traits: {len(self.mbti_comments)}")
        print(f"Total comments from authors with all traits: {len(self.all_traits_comments)}")
        
    def get_top_bottom_percentile_comments(self, trait, percentile, comments, top=True):
        sorted_authors = self.author_profiles[~self.author_profiles[trait].isnull()].sort_values(by=trait, ascending=not top)
        total_authors = len(sorted_authors)
        num_authors = int(np.ceil((percentile / 100) * total_authors))
        percentile_authors = sorted_authors['author'].head(num_authors) if top else sorted_authors['author'].tail(num_authors)
    ...
    def perform_splitting(self):
        ...
        for k in self.k_values:
            ...
            for trait in self.ocean_traits:
                top_k_comments = self.get_top_bottom_percentile_comments(trait, k, self.ocean_comments, top=True)
                bottom_k_comments = self.get_top_bottom_percentile_comments(trait, k, self.ocean_comments, top=False)
                top_k_comments_dict[trait] = top_k_comments
                bottom_k_comments_dict[trait] = bottom_k_comments
            min_top_comments_count = min(len(top_k_comments_dict[trait]) for trait in self.ocean_traits)
            min_bottom_comments_count = min(len(bottom_k_comments_dict[trait]) for trait in self.ocean_traits)
            ...
            for trait in self.ocean_traits:
                balanced_top_k_comments = top_k_comments_dict[trait].sample(n=min_comment_count, random_state=42)
                balanced_bottom_k_comments = bottom_k_comments_dict[trait].sample(n=min_comment_count, random_state=42)
                ...
                balanced_top_k_comments.to_csv(top_file_path, index=False)
                bottom_file_path = os.path.join(current_split_folder, bottom_file_name)
                balanced_bottom_k_comments.to_csv(bottom_file_path, index=False)
                print(f"Saved balanced top {k}% comments for {trait} to {top_file_path}")
                print(f"Saved balanced bottom {k}% comments for {trait} to {bottom_file_path}")
            time.sleep(1)
    
    def save_comments(self):
        print("Saving filtered comments...")
        ocean_comments_path = os.path.join(self.data_folder, 'filtered_ocean_comments.csv')
        mbti_comments_path = os.path.join(self.data_folder, 'filtered_mbti_comments.csv')
        all_traits_comments_path = os.path.join(self.data_folder, 'filtered_all_traits_comments.csv')
        self.ocean_comments.to_csv(ocean_comments_path, index=False)
        self.mbti_comments.to_csv(mbti_comments_path, index=False)
        self.all_traits_comments.to_csv(all_traits_comments_path, index=False)
...
if __name__ == "__main__":
    ...
    current_folder = workspace_path + """thesis/Using Task Arithmetic with PEFT for LMs' personality assessment/"""
    data_folder = current_folder + 'data/pandora/'
    comments_file = data_folder + 'all_comments_since_2015.csv'
