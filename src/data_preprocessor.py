...
"""Preprocess raw text data, clean comments, and build balanced splits."""
...
class DataPreprocessor:
    ...
    def __init__(
        ...
        splits_folder_balanced: str | None = None,
        ocean_traits: list[str] | None = None,
        mbti_traits: list[str] | None = None,
        k_values: list[int] | None = None,
    ) -> None:
        """Initialize the preprocessor with dataset paths and trait settings."""
    ...
    def load_data(self) -> None:
        """Load comments and author profile tables from CSV files."""
    ...
    def clean_comments(self) -> None:
        """Clean comment text and normalize encoding artifacts."""
    ...
    def filter_authors_with_profiles(self) -> None:
        """Keep only comments from authors with profile data."""
    ...
    def filter_authors_with_complete_traits(self) -> None:
        """Split comments by authors with complete OCEAN and MBTI traits."""
    ...
    def get_top_bottom_percentile_comments(
        self, trait: str, percentile: int, comments: pd.DataFrame, top: bool = True
    ) -> pd.DataFrame:
        """Return comments for the top or bottom percentile of a trait."""
        sorted_authors = self.author_profiles[~self.author_profiles[trait].isnull()].sort_values(
            by=trait, ascending=not top
        )
    ...
    def perform_splitting(self) -> None:
        """Create balanced top and bottom percentile splits for each trait."""
    ...
    def save_comments(self) -> None:
        """Save filtered comment tables to CSV files."""
