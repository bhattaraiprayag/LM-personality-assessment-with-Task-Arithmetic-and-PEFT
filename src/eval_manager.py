# src/eval_manager.py
"""Evaluate language models with Likert-style inventory prompts."""
...
def get_inventory(name: str) -> dict:
    """Return a normalized inventory configuration."""
...
class EvalManager:
    """Score a language model against inventory prompts."""
    ...
    def __init__(
        ...
        """Initialize the model, tokenizer, and device for evaluation."""
    ...
    def score_likert(
        ...
        *,
        ...
        include_options: bool | str = "both",
        batch_size: int = 5,
        return_dataframe: bool = True,
    ) -> pd.DataFrame | torch.Tensor:
        """Score an inventory and return probabilities or a DataFrame."""
        ...
        anchor_ids = [self.tokenizer(" " + a, add_special_tokens=False).input_ids for a in anchors]
    ...
    def _build_prompt(
        ...
        """Build a single prompt for an item/anchor combination."""
        if include_options:
    ...
    def _collate_batch(
        ...
        *,
        ...
        """Collate prompts and anchors into model inputs."""
        prompts = [
            self._build_prompt(item, anchors, question_stem, statement_template, include_options)
    ...
    def _forward_avg_loss(self, toks: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """Compute average masked loss for each prompt/answer row."""
    ...
    def _avg_loss_to_probs(self, avg_loss: torch.Tensor, batch_items: int) -> torch.Tensor:
        """Convert per-answer losses into normalized probabilities."""
