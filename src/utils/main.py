...
class Utilities:
    ...
    def find_max_batch_size(
        model: torch.nn.Module, tokenizer, device: torch.device, args: ExperimentArguments
    ) -> int:
    ...
    @staticmethod
    def find_optimal_lr(
        model: LightningModule, train_loader: torch.utils.data.DataLoader, args: ExperimentArguments
    ) -> float:
    ...
    def create_loggers(output_dir: str, experiment_id: str, args: ExperimentArguments) -> tuple:
        ...
        """
        tb_name = "tb_logs"
        wandb_project = "Independent Research Project"
