...
"""Benchmark perplexity for language models on small reference corpora."""
...
def tokenize_function(
    examples: dict[str, list[str]], tokenizer: AutoTokenizer, text_column: str
...
def group_texts(examples: dict[str, list[list[int]]]) -> dict[str, list[list[int]]]:
    """Chunk tokenized examples into fixed-length language-model blocks."""
...
def benchmark_perplexity(
    model: AutoModelForCausalLM, tokenizer: AutoTokenizer, args
) -> dict[str, float]:
    """Compute perplexity scores for the built-in reference datasets."""
    set_seed(args.seed)
...
def run_perplexity_benchmark(model, tokenizer, seed=42):
    """Run the perplexity benchmark with a minimal seed-only args object."""

    class Args:
        """Container for the benchmark seed."""
