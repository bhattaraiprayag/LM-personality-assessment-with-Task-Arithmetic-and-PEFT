...
def initialize_model(args: Namespace, data_manager: DataManager, warmup_steps: int) -> CLMModel:
...
def calculate_warmup_steps(
    args: Namespace, data_manager: DataManager, device_config: dict[str, Any]
) -> int:
    ...
    total_steps = (
        ...
        if not device_config["multi_gpu"]
            ...
            (len_train_loader * args.epochs * len(device_config["devices"])) / args.grad_steps
        )
    )
